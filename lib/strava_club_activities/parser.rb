  ...
  class Parser
    ...
    def parse
      @raw_data.each do |run|
        @runs << StravaClubActivities::Run.new(run)
      end
    end

    def to_csv(filename = 'test.csv')
      CSV.open(filename, 'w') do |csv|
        csv << %w[athlete run_title average_pace_(min/km) distance_(km) moving_time elapsed_time moving_% elevation_(m) raw_distance raw_moving_time raw_elapsed_time]
